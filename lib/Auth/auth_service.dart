...
contentsUpdate(user,_photo,TitleController,contentsController,priceController,url)async{
  ...
  if (task != null) {
   ...
   LikeFunction(like,id,user) async {  // 좋아요 기능
    ...
    Wishlist(user,TitleController,contentsController,priceController,url,wish) {
      var doc = FirebaseFirestore.instance.collection('${FirebaseAuth.instance.currentUser!.displayName!}Wish').doc(priceController.text); 
      doc.set({
        'id': doc.id,
        'displayName':FirebaseAuth.instance.currentUser!.displayName!,
        'title' : TitleController.text,
    ...
    wishupdate(user,priceController,num,list){ //
      ...
      if(num == 0){
        doc.update({
        'wish' : list
         }).whenComplete(() => print('위시 성공'));
      } else {
        doc.update({
        'wish' : list
        }).whenComplete(() => print('위시 삭제'));
      }
}
